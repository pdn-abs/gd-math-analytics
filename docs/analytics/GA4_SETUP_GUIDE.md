...
### 3.2 Security Setup
```bash
# Create .gitignore for analytics folder (append safely)
mkdir -p analytics
grep -qxF "keys/" analytics/.gitignore 2>/dev/null || printf "keys/\n" >> analytics/.gitignore

# Secure keys permissions
chmod 600 analytics/keys/*.json
...
# Add this line for daily 6 AM updates (repository-relative path):
# 0 6 * * * cd $HOME/path/to/repo && cd analytics && source setup_credentials.sh && cd scripts && node fetch.js
```

### 7.2 Node-cron Integration
See `docs/node_cron_setup.md` for automated scheduling within Node.js

### 7.3 Running automation (no pm2)

If you don't want to use `pm2`, run the automation in background with `nohup` or schedule it with `cron`.

Background with `nohup`:

```bash
mkdir -p analytics/logs
nohup node analytics/scripts/automate.js > analytics/logs/automate.out.log 2>&1 &
```
...
0 6 * * * cd /path/to/repo && source analytics/setup_credentials.sh && node analytics/scripts/fetch.js >> analytics/logs/cron.log 2>&1
```
